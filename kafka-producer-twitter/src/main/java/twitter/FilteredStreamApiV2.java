 ...
 * */
public class FilteredStreamApiV2 {
    ...
    public static void main(String args[]) throws IOException, URISyntaxException, InterruptedException {
        ...
        if (null != bearerToken) {
            ...
            if(false){
                ...
                new Thread(new TwitterProducerThread(msgQueue,bearerToken,rules,500)).start();
                String msg = null;
                while (true){
    ...
    private static void connectStream(String bearerToken) throws IOException, URISyntaxException, InterruptedException {
     ...
     static void connectStreamToBQ(String bearerToken, BlockingQueue<String> bq, long ms, AtomicBoolean ab) throws IOException, URISyntaxException, InterruptedException {
        ...
        if (null != entity) {
            BufferedReader reader = new BufferedReader(new InputStreamReader((entity.getContent())));
            String line = reader.readLine();
            while (line != null && ab.get()) {
                //Thread.sleep(ms);
                bq.put(line);    //It waits that the consumer free a slot in the queue
                //bq.add(line); //It throws an exception if the queue is full
                line = reader.readLine();
            }
        }
     ...
     * */
     static void setupRules(String bearerToken, Map<String, String> rules) throws IOException, URISyntaxException {
